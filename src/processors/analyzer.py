...
def merge_transactions_inventory(df_tx: pd.DataFrame, df_inv: pd.DataFrame) -> pd.DataFrame:
    df_tx = df_tx.copy()
    df_inv = df_inv.copy()
...
def add_rolling_revenue(df: pd.DataFrame, window: int = 30) -> pd.DataFrame:
    df = df.copy()
    df['30d_avg_revenue'] = (
        df.groupby(['region', 'item_sku'])['gross_revenue']
...
def get_top_performers(df: pd.DataFrame, quantile: float = 0.9) -> pd.DataFrame:
    revenue_by_item = (
        df.groupby(['region', 'item_sku'], as_index=False)['gross_revenue']
        .sum()
    )
    ...
    return revenue_by_item.groupby('region', group_keys=False).apply(top_percent)