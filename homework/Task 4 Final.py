...
def generate_transactions(num_records):
    ...
    random_dates = [
        START_DATE + timedelta (days=random.randint(1, time_delta.days))
        for _ in range(num_records)
    ]

    data = {
        'transactions': range(10000, 10000+num_records),
...
df_tx['gross_revenue'] = df_tx['unit_price'] * df_tx['unit_sold']
...
top_performers = (
    revenue_by_item
    .gropby('region', group_keys=False)
